import { CreateRegistroEnvaseRequest, RegistroEnvaseResponse, ApiError } from '../types/api';
...
class ProductionRecordsService {
    ...
    try {
      ...
      if (error instanceof Error) {
        if (error.name === 'AbortError') {
          throw new Error('La petición ha excedido el tiempo límite. Verifique su conexión.');
        }
  ...
  async createRegistroEnvase(data: CreateRegistroEnvaseRequest): Promise<RegistroEnvaseResponse> {
    return this.makeRequest<RegistroEnvaseResponse>('/api/production-records/envases', {
      method: 'POST',
...
export const validateRegistroEnvase = (data: Partial<CreateRegistroEnvaseRequest>) => {
  const errors: string[] = [];
  ...
  if (!data.cantidadDeEnvasesProducidos || data.cantidadDeEnvasesProducidos <= 0) {
